...
import com.ahmed3v.bookslibrary.BaseTest.DrawableMatcher.withDrawable
...
open class BaseTest {
    ...
    val lastPosition = DataSource.Books.size - 1

    /**
     * Check the content of a card
     *
     * @param name
     * @param author The full author string as it appears on the screen
     * @param rating The full rating string as it appears on the screen
     * @param imageResource The image resource Id
     */
    private fun hasListItemContent(name: String, author: String, rating: String, imageResource: Int) {
        onView(withText(name))
            .check(matches(isDisplayed()))
        onView(withText(author))
            .check(matches(isDisplayed()))
